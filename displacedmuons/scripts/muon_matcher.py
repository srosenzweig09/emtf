...
for evt in range(nevents):
    ...
    evt_muons = muon[evt]
    ngen = len(nptab[prefix + '_pt'][evt][evt_muons])
    ...
    # gen muons
    mu_eta = nptab[prefix + '_eta'][evt][evt_muons]
    mu_phi = nptab[prefix + '_phi'][evt][evt_muons]
    mu_vx = nptab[prefix + '_vx'][evt][evt_muons]
    mu_vy = nptab[prefix + '_vy'][evt][evt_muons]
    mu_vz = nptab[prefix + '_vz'][evt][evt_muons]
...
# gen_indices = ak.fromiter(gen_indices)
# emtf_indices = aw.fromiter(emtf_indices)
...
# try:
#     ak.persist.save(f"matching/{sample}_gen_ind.awkd", gen_indices)
#     ak.persist.save(f"matching/{sample}_emtf_ind.awkd", emtf_indices)
# except:
