...
void PtAssignment::process(EMTFTrackCollection& best_tracks) {
  ...
  for (; best_tracks_it != best_tracks_end; ++best_tracks_it) {
    ...
    gmt_dxy    = aux_dxy().getGMTDxy(dxy);

    pt_dxy = aux_dxy().getPtFromGMTPtDxy(gmt_pt_dxy);
...
const PtAssignmentEngineDxyAux& PtAssignment::aux_dxy() const { return pt_assign_engine_dxy_->aux(); }
